...
function prism(height, sidesLength, numOfSides) {
   var coords = [];
   var normals = [];
   var texCoords = [];
   var indices = [];
   ...
   var angle = (2 * Math.PI) / numOfSides;
 
   // Generate the top and bottom vertex positions.
   for (var i = 0; i < 2; i++) {
     var y = i === 0 ? -height / 2 : height / 2;
     for (var j = 0; j < numOfSides; j++) {
       var x = Math.cos(j * angle) * sidesLength;
       var z = Math.sin(j * angle) * sidesLength;
       coords.push(x, y, z);
     }
   }
   
   // Generate normals.
   for (var i = 0; i < 2 * numOfSides; i++) {
     normals.push(0, i < numOfSides ? -1 : 1, 0);
   }
   
   // Generate texture coordinates (simplistic)
   for (var i = 0; i < 2 * numOfSides; i++) {
     texCoords.push(i % 2, Math.floor(i / 2));
   }
   
   // Generate indices for the top and bottom faces
   for (var i = 0; i < numOfSides - 2; i++) {
     indices.push(0, i + 1, i + 2);
     indices.push(numOfSides, numOfSides + i + 1, numOfSides + i + 2);
   }
 
   // Generate indices for the side faces.
   for (var i = 0; i < numOfSides; i++) {
     var lower1 = i;
     var lower2 = (i + 1) % numOfSides;
     var upper1 = i + numOfSides;
     var upper2 = (i + 1) % numOfSides + numOfSides;
 
     indices.push(lower1, lower2, upper1);
     indices.push(lower2, upper2, upper1);
   }
 ...
 function pyramid(baseLength, height) {
   var coords = [];
   var normals = [];
   var texCoords = [];
   var indices = [];
   ...
   // Vertex positions
   // Base vertices
   coords.push(-halfBase, 0, -halfBase);  // 0
   coords.push(halfBase, 0, -halfBase);   // 1
   coords.push(halfBase, 0, halfBase);    // 2
   coords.push(-halfBase, 0, halfBase);   // 3
   // Top vertex
   coords.push(0, height, 0);  // 4
 
   // Normals - these would actually be calculated, but for this example
   // we'll just have them point upwards and downwards
   normals.push(0, -1, 0);
   normals.push(0, -1, 0);
   normals.push(0, -1, 0);
   normals.push(0, -1, 0);
   ...
   // Texture coordinates - for simplicity
   texCoords.push(0, 0);
   texCoords.push(1, 0);
   texCoords.push(1, 1);
   texCoords.push(0, 1);
   texCoords.push(0.5, 0.5); // Top vertex
 ...
 function dodecahedron(sideLength) {
   ...
   // Define vertices
   // 20 vertices defined by the cartesian coordinates
   var rawVertices = [
   ...
   // Scale vertices so they are all the same distance from the origin
   for (let vertex of rawVertices) {
     var length = Math.sqrt(vertex[0] ** 2 + vertex[1] ** 2 + vertex[2] ** 2);
     coords.push(...vertex.map(x => x / length * sideLength));
     normals.push(...vertex.map(x => x / length)); // Simplified normals
   }
 
   // Simplified texture coords
   for (let i = 0; i < 20; i++) {
   ...
   // Define indices
   var rawIndices = [
     [0, 12, 14, 2, 10], [0, 10, 11, 1, 6], [0, 6, 7, 3, 12],
     [8, 4, 14, 12, 16], [8, 16, 17, 5, 13], [8, 13, 15, 2, 4],
     [9, 3, 7, 5, 18], [9, 18, 19, 1, 11], [9, 11, 10, 2, 15],
     [7, 6, 1, 19, 18], [3, 9, 15, 13, 12], [13, 5, 17, 16, 12]
   ];
 
   for (let face of rawIndices) {
     indices.push(...face.slice(0, 3));
     indices.push(...face.slice(2, 5).reverse());
   }
 ...
 function icosahedron(sideLength) {
   ...
   for (let vertex of rawVertices) {
     let length = Math.sqrt(vertex[0] ** 2 + vertex[1] ** 2 + vertex[2] ** 2);
     coords.push(...vertex.map(x => x / length * sideLength));
     normals.push(...vertex.map(x => x / length));
   }
   ...
   // Define indices for the 20 faces
   var rawIndices = [
